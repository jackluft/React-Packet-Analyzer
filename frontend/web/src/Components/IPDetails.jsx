...
function IPDetails({ip, data}){
    const countryCodeToName = {
        ...
        "BN": "Brunei Darussalam", "BO": "Bolivia", "BQ": "Caribbean Netherlands", "BR": "Brazil", "BS": "Bahamas",
        "BT": "Bhutan", "BV": "Bouvet Island", "BW": "Botswana", "BY": "Belarus", "BZ": "Belize",
        "CA": "Canada", "CC": "Cocos (Keeling) Islands", "CD": "Congo (DRC)", "CF": "Central African Republic",
        "CG": "Congo (Republic)", "CH": "Switzerland", "CI": "Côte d'Ivoire", "CK": "Cook Islands", "CL": "Chile",
        ...
        "ET": "Ethiopia", "FI": "Finland", "FJ": "Fiji", "FM": "Micronesia", "FO": "Faroe Islands", "FR": "France",
    ...
    return <div className="ip-container">
        <h1 className="ip-header">IP address: {ip}</h1>
        <div className="ip-details">
            <span></span>
            <p className="ip-txt"> <span className="ip-title">Country:</span>  {countryCodeToName[data.country]} {countryCodeToFlagEmoji(data.country)}</p>
            <p className="ip-txt"><span className="ip-title" >City:</span> {data.city}</p>
            <p className="ip-txt"><span className="ip-title">Region:</span> {data.region}</p>
            <p className="ip-txt"><span className="ip-title">Network provider:</span> {data.org
            }</p>
        </div>
    </div>
